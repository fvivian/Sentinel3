	...
	'''
        ...
        if createNewDir:
            ...
            try:
                ...
                print('Create directory with the name %s and saved .png file in it.' %(self.targetDir))
            except:
                print('Directory with the name %s already exists, .png file saved in it.' %(self.targetDir))
        ...
        if createNewDir:
            self.pngDir = os.path.join(self.targetDir, self.out)+'.png'
            plt.savefig(self.pngDir, transparent=True, bbox_inches=extent)
        else:
            self.pngDir = self.out+'.png'
            plt.savefig(self.pngDir, transparent=True, bbox_inches=extent)
        plt.close('all')
